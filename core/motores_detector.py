"""Motor 0 — Detector de Origem de Documento.
...
class DetectorDocumento:
    ...
    def detectar(self, filepath: Path) -> TipoDocumento:
        ...
        # XML: verifica se é NF-e pelo conteúdo
        if sufixo in self.EXTENSOES_XML:
            if self._e_nfe(filepath):
                return TipoDocumento.NFE_XML
            # XML genérico — tratar como não suportado por ora
    ...
    def _e_nfe(self, filepath: Path) -> bool:
        """Verifica se o XML é uma NF-e válida pela tag raiz."""
        try:
            # Lê apenas os primeiros 2KB — suficiente para a tag raiz
    ...
    def _pdf_tem_texto(self, filepath: Path) -> bool:
        """Verifica se o PDF tem camada de texto extraível."""
        try:
            import pdfplumber
            with pdfplumber.open(filepath) as pdf:
                for page in pdf.pages[:3]:  # verifica as 3 primeiras páginas
